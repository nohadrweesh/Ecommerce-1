
$(document).ready(function(){
	$('#new_pwd').click(function(){
		var current_pwd=$('#current_pwd').val();
		$.ajax({
			type:'get',
			url:'/admin/check-pwd',
			data:{current_pwd:current_pwd},
			success:function (response) {
				if(response=="false"){
                    $("#current_pwd").parents('.control-group').removeClass('success');
                    $("#current_pwd").parents('.control-group').addClass('error');
                    $("#chkPwd").html("<font style=\"color:red;\">Current password is Incorrect</font>")

				}

				else if(response=="true") {
                    $("#current_pwd").parents('.control-group').removeClass('error');
                    $("#current_pwd").parents('.control-group').addClass('success');
                    $("#chkPwd").html("<font style=\"color:green;\">Current password is Correct</font>")
                }
            },
			error:function () {
				alert("Error");
            }
		})

	});
	
	//$('input[type=checkbox],input[type=radio],input[type=file]').uniform();
	
	//$('select').select2();

	$("#password_validate").validate({
		rules:{
			current_pwd:{
				required: true,
				minlength:6,
				maxlength:20
			},
			new_pwd:{
				required: true,
				minlength:6,
				maxlength:20
			},
			confirm_pwd:{
				required:true,
				minlength:6,
				maxlength:20,
				equalTo:"#new_pwd"
			}
		},
		errorClass: "help-inline",
		errorElement: "span",
		highlight:function(element, errorClass, validClass) {
			$(element).parents('.control-group').addClass('error');
		},
		unhighlight: function(element, errorClass, validClass) {
			$(element).parents('.control-group').removeClass('error');
			$(element).parents('.control-group').addClass('success');
		}
	});
	$("#add_category").validate({
		rules:{
			category_name:{
				required:true
			},
            category_url:{
                required:true,
				//url:true
            },
		},
        errorClass: "help-inline",
        errorElement: "span",
        highlight:function(element, errorClass, validClass) {
            $(element).parents('.control-group').addClass('error');
        },
        unhighlight: function(element, errorClass, validClass) {
            $(element).parents('.control-group').removeClass('error');
            $(element).parents('.control-group').addClass('success');
        }

	});
	$("#edit_category").validate({
		rules:{
			category_name:{
				required:true
			},
            category_url:{
                required:true,
				//url:true
            },
		},
        errorClass: "help-inline",
        errorElement: "span",
        highlight:function(element, errorClass, validClass) {
            $(element).parents('.control-group').addClass('error');
        },
        unhighlight: function(element, errorClass, validClass) {
            $(element).parents('.control-group').removeClass('error');
            $(element).parents('.control-group').addClass('success');
        }

	});

    $(document).on('click','.deleteRecord',function(e){
        var id = $(this).attr('rel');
        var deleteFunction = $(this).attr('rel1');
        swal({
                title: "Are you sure?",
                text: "Your will not be able to recover this Record Again!",
                type: "warning",
                showCancelButton: true,
                confirmButtonClass: "btn-danger",
                confirmButtonText: "Yes, delete it!",
                //closeOnConfirm: false
         }).then((value)=>{
            window.location.href="/admin/"+deleteFunction+"/"+id;
		});
            // function(){
        		// alert("hii");
            //     window.location.href="/admin/"+deleteFunction+"/"+id;
            // });
    });



var maxField = 10; //Input fields increment limitation
var addButton = $('.add_button'); //Add button selector
var wrapper = $('.field_wrapper'); //Input field wrapper
var fieldHTML = '<div> <input style="margin-left:200px; margin-top: 5px;margin-right: 3px;"required type="text" name="sku[]" placeholder="sku"/>' +
    '<input style=" margin-top: 5px;margin-right: 3px;" required type="text" name="size[]" placeholder="size"/>' +
    '<input style=" margin-top: 5px;margin-right: 3px;" required type="text" name="price[]" placeholder="price"/>' +
    '<input style=" margin-top: 5px;margin-right: 3px;" required type="text" name="stock[]" placeholder="stock"/>' +
	'<a href="javascript:void(0);" class="remove_button">Remove</a></div>'; //New input field html
var x = 1; //Initial field counter is 1

//Once add button is clicked
$(addButton).click(function(){
	//Check maximum number of input fields
	if(x < maxField){
		x++; //Increment field counter
		$(wrapper).append(fieldHTML); //Add field html
	}
});

//Once remove button is clicked
$(wrapper).on('click', '.remove_button', function(e){
	e.preventDefault();
	$(this).parent('div').remove(); //Remove field html
	x--; //Decrement field counter
});



});
